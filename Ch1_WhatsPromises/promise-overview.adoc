...
<2> 调用 promise 对象的 `.then` 方法来注册操作完成时的回调函数。

`asyncFunction` 这个函数会返回 promise 对象，
对于这个 promise 对象，我们调用它的 `then` 方法来注册 resolve 后的回调函数，
`catch` 方法来注册发生错误时的回调函数。

该 promise 对象会在 `setTimeout` 之后的 16ms 时被 resolve,
...
若不使用 `catch` 而仅使用 `then` 方法的话，也能完成相同的工作，代码如下所示的：
